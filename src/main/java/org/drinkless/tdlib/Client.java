 ...
 * Based on official TDLib Java example.
 */
public final class Client {
    static {
        // Library is loaded by TdLib.kt
    }
    ...
    public static class ExecutionException extends Exception {
        /**
         * Original TDLib error.
         */
        public final TdApi.Error error;

        /**
         * Creates ExecutionException.
         *
         * @param error TDLib error.
         */
    ...
    private static class ResponseReceiver implements Runnable {
        @Override
        public void run() {
            int[] clientIds = new int[1000];
            long[] eventIds = new long[1000];
            TdApi.Object[] events = new TdApi.Object[1000];
        ...
        private void processResult(int clientId, long id, TdApi.Object object) {
            boolean isClosed = object instanceof TdApi.UpdateAuthorizationState &&
                    ((TdApi.UpdateAuthorizationState) object).authorizationState instanceof TdApi.AuthorizationStateClosed;
            Handler handler = null;
            if (id != 0) {
                handler = handlers.remove(id);
            } else {
                handler = updateHandlers.get(clientId);
            }
            if (handler != null) {
                try {
            ...
            if (isClosed) {
                ...
                if (clientCount.decrementAndGet() == 0) {
                    // No more clients - can stop receiver thread
                }
            }
        }
    }

    final int nativeClientId;
    ...
    private static void startReceiverThread() {
        if (receiverThread == null) {
            receiverThread = new Thread(responseReceiver, "TDLib-Receiver");
    ...
    // Native methods - these match what tdjni.dll exports
